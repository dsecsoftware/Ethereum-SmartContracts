      ...
      <v-container>
        ...
        <v-layout row justify-center>
          <v-dialog v-model="startProjectDialog" max-width="600px" persistent>
            <v-btn slot="activator" color="primary" dark>Starte dein Projekt</v-btn>
            <v-card>
              <v-card-title>
                <span class="headline font-weight-bold mt-2 ml-4">Lasse dein Projekt auferstehen!</span>
              </v-card-title>
              ...
              >
                <v-card-title primary-title>
                  <div>
                    ...
                    <br/><br/>
                    <small>Läuft bis: <b>{{ new Date(project.deadline * 1000) }}</b></small>
                    <br/><br/>
                    <small>Ziel von <b>{{ project.goalAmount / 10**18 }} ETH </b></small>
                    <small v-if="project.currentState == 1">konnte leider nicht in der angegebenen Zeit erreicht werden</small>
                    <small v-if="project.currentState == 2">wurde erreicht</small>
                  </div>
                </v-card-title>
                <v-flex
...
export default {
  ...
  data() {
    return {
      ...
      account: null,
      stateMap: [
        { color: 'primary', text: 'Läuft' },
        { color: 'warning', text: 'Abgelaufen' },
        { color: 'success', text: 'Erfolgreich' },
      ],
      projectData: [],
      newProject: { isLoading: false },
    };
  ...
  methods: {
    ...
    fundProject(index) {
      ...
      projectContract.methods.contribute().send({
        ...
        const expired = res.events.ProjectExpired.returnValues.expired;
        ...
        // Set project state to expired
        if(expired == true) {
          this.projectData[index].currentState = 1;
        }
        // Set project state to success
        else if (newTotal >= projectGoal) {
